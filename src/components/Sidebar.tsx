...
import { LayoutDashboard, History, Users, Settings, LogOut, Check, Heart, X } from 'lucide-react';
...
export default function Sidebar({
  ...
  const SidebarContent = () => (
    <div className="h-full flex flex-col py-6 bg-white border-r border-slate-200/80 shadow-sm">
      {/* Brand Header */}
      <div className="flex items-center gap-3 px-6 mb-10 select-none">
        <div className="w-10 h-10 rounded-xl bg-blue-50 flex items-center justify-center border border-blue-100">
          <Heart className="text-[#0053db] fill-[#0053db]/10 w-5.5 h-5.5" />
        </div>
        <div>
        ...
        {isOpenMobile && (
          <button 
            ...
            className="ml-auto p-1.5 rounded-lg text-slate-400 hover:text-slate-600 hover:bg-slate-50 transition-all md:hidden"
          >
            <X className="w-5 h-5" />
          </button>
        )}
        ...
        {menuItems.map((item) => {
          ...
          return (
            <button
              ...
              className="w-full text-left group flex items-center gap-3.5 px-4 py-3 rounded-2xl relative transition-all duration-200"
            >
              {isActive && (
                <motion.div
                  layoutId="activeBar"
              ...
              <div className={`relative z-10 p-2 rounded-xl transition-all duration-200
                ${isActive 
                  ...
                  : 'bg-slate-50 border border-slate-200 text-slate-400 group-hover:text-slate-600 group-hover:bg-slate-100'
                }`}
              >
      ...
      <div className="px-4 mt-auto pt-6 border-t border-slate-100 space-y-1.5">
        <button
          ...
          className={`w-full text-left group flex items-center gap-3.5 px-4 py-3.5 rounded-2xl relative transition-all duration-200
            ${activeTab === 'settings' ? 'bg-blue-50/50 border border-blue-100' : 'hover:bg-slate-50'}
          `}
        >
          <div className={`p-2 rounded-xl border transition-all duration-200
            ${activeTab === 'settings' 
              ? 'bg-blue-600 text-white shadow-md border-blue-600' 
              : 'bg-slate-50 border-slate-250 text-slate-400 group-hover:text-slate-600 group-hover:bg-slate-100'
            }`}
          >
          ...
          <div className="select-none">
            <span className={`text-sm font-semibold block transition-colors duration-200
              ${activeTab === 'settings' ? 'text-blue-700' : 'text-slate-600 group-hover:text-slate-900'}
            `}>
              Pengaturan
            </span>
        ...
        <button
          onClick={onLogout}
          className="w-full text-left group flex items-center gap-3.5 px-4 py-3.5 rounded-2xl hover:bg-red-50/60 transition-all duration-200"
        >
          ...
          <div className="select-none">
            <span className="text-sm font-semibold text-slate-600 group-hover:text-red-700 block transition-colors duration-200 col-span-1">
              Logout
            </span>
  ...
  return (
    <>
      ...
      <aside className="hidden md:flex flex-col h-screen w-72 shrink-0 sticky left-0 top-0 overflow-y-auto">
        <SidebarContent />
      </aside>
